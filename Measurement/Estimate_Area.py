...
def getROI(image, mask): 
    ...
    for i, c in enumerate(contours):
        if hierachy[0][i][3] == -1:
            ...
            cv2.drawContours(polyRectImage, [box], 0, (255,0,0),2)
            #show(polyRectImage, "Poly Rectangle")
            
            # Get Roundness
            # print("minRect data: " + str(minRect))
            # print("boundRect data: " +str(boundRect))
            minArea = minRect[1][0] * minRect[1][1]
            badArea = boundRect[2] * boundRect[3]
    ...
    return croppedImg, contourArea, minArea, badArea
...
def test():
    ...
    image3, contourArea, minArea, badArea = getROI(image2, image1)
    # plt.imshow(images[0], cmap='gray', vmin=0, vmax=255)
    # plt.show()
    # plt.imshow(images[1], cmap='gray', vmin=0, vmax=255)
    # plt.show()
    roundness = contourArea / minArea
    titles = ['Original Image', 'Hue Thresholding', 'Masked','ROI cuadrado']
    images = [inputImage, image1, image2, image3]
    ...
    plt.text(0, -0.075, "Contour Area= {}".format(contourArea), ha="center", transform=plt.gca().transAxes)
    plt.text(0, -0.15, "Minimum rectangle area= {}".format(minArea), ha="center", transform=plt.gca().transAxes)
    plt.text(0, -0.225, "Straight rectangle area= {}".format(badArea), ha="center", transform=plt.gca().transAxes)
    plt.text(0, -0.3, "Roundness Factor= {}".format(round(roundness, 3)), ha="center", transform=plt.gca().transAxes)
    plt.show()
